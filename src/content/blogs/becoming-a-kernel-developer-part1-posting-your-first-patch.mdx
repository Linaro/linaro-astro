...
2. **Body**: After the subject line comes a blank line, followed by the body of the message. Here you explain your change in detail. Focus on the "why", rather than the "how":
   1. Describe the problem. Why is this change needed? What was the problematic behavior?
   2. Describe your solution. How does your change fix the problem? Explain the logic of your approach.
   3. Describe any testing you performed. This helps build confidence that your patch will not introduce new regressions.
3. **Tags**: At the end of the commit message there are machine-readable tags:

   1. Signed-off-by: `"Your RealName <your.email@example.com>"` <br> This tag is always required at the end of the patch. With this line you certify that you wrote the patch yourself or have the right to pass it on as an open-source patch. Please read the instructions and the “Developer’s Certificate of Origin” carefully before adding this line.
   2. Fixes: `<commit hash> (“original commit message”)` <br> If you fix a bug, please inspect the Git history to determine which commit originally introduced the bug and mention it in your commit message. This simplifies backporting the fix to stable releases later. The “git blame” command is often helpful for this.
   3. Reported-by: `"Reporter RealName <reporter.email@example.com>"` <br> Use this to give credit to the person who initially reported the bug.
   4. Link: https://lore.kernel.org/r/.../ <br> Use this to link to prior discussions or bug reports.

4. **Footer**: Everything written below a “---” line in the commit message will not end up getting applied to the kernel commit log. Use this space to provide additional comments about your patch. If you are uncertain about a certain part of your patch or would like to ask questions, indicate this here.
