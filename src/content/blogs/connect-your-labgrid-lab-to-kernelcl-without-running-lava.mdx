...
FIGURE 1 GOES HERE. The flow it shows: a KernelCI job is queued; the Linaro Automation Appliance polls for it, downloads and deploys; it routes to the configured backend, either Labgrid using pytest fixtures or tuxrun; that drives real hardware over power control, serial console and network boot; results return to KernelCI as JUnit XML via the callback API.
