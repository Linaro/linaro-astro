...
date: 2024-04-17T08:05:14.000Z
image: >-
  https://res.cloudinary.com/dl7c2wmhi/image/upload/v1758792170/linaro-website/images/blog/Tech_Background
tags:
  - testing
  - linux-kernel
  - toolchain
...
TuxMake provides Docker container images to build Linux kernels across various architectures and toolchain combinations. These container images are Debian-based (ranging from oldstable to unstable) and come pre-installed with stock (Debian-provided) toolchains and all the tools required for kernel compilation. The TuxMake Docker container images ([https://hub.docker.com/u/tuxmake](https://hub.docker.com/u/tuxmake)) can be used in any Linux distribution to build Linux kernels.
...
The stock toolchains that are part of the Debian repositories are generic and not optimised for the linux kernel builds or for any project since they have to cater to all projects as a compiler toolchain provided by the Debian operating system. In the case of the Linux kernel project, the official website provides compilers that are optimised (most of the time) for building the Linux kernel. The users of TuxMake have [wished](https://gitlab.com/Linaro/tuxmake/-/issues/215) to leverage the toolchains provided by kernel.org for a [long time](https://gitlab.com/Linaro/tuxmake/-/issues/99), and finally, the wait is over! TuxMake has introduced  Docker containers that have pre-installed toolchains from kernel.org ([https://mirrors.edge.kernel.org/pub/tools/llvm/](https://mirrors.edge.kernel.org/pub/tools/llvm/)).

## Internals of kernel.org LLVM toolchain support

The TuxMake kernel.org toolchain Docker images use a Debian base with all the necessary tools to build a Linux kernel for a specific architecture, except for the compiler toolchain itself which is obtained from [https://mirrors.edge.kernel.org/pub/tools/llvm/](https://mirrors.edge.kernel.org/pub/tools/llvm/).

These Docker images are available in two host architecture combinations (aarch64 and x86\_64). Figure 1, shows the supported kernel.org toolchains and the target architectures.

![Tuxmake kernel.org toolchain vs architecture support matrix](https://res.cloudinary.com/dl7c2wmhi/image/upload/v1758792170/linaro-website/images/blog/figure-1-tuxmake-blog)

NOTE: The loongarch target architecture Docker image is only available for x86\_64 host architecture.
...
When necessary, the container for loongarch\_korg-clang-18 will be fetched from TuxMake's public container registry at [hub.docker.com/u/tuxmake](https://hub.docker.com/u/tuxmake).
...
Thus TuxMake makes building Linux kernels with official kernel.org toolchains a breeze. The TuxMake developers are actively working to bring in support for GCC, which is available at [https://mirrors.edge.kernel.org/pub/tools/crosstool/](https://mirrors.edge.kernel.org/pub/tools/crosstool/).
...
Please open an issue at [https://gitlab.com/Linaro/tuxmake/](https://gitlab.com/Linaro/tuxmake/), and email us at tuxsuite@linaro.org if you have any concerns. The complete documentation of TuxMake is available at [https://docs.tuxmake.org/](https://docs.tuxmake.org/)
